...
export class FlowAdapter {
 ...
 async uploadReference(file){
  ...
  const dt=new this.DataTransferCtor();dt.items.add(file);input.files=dt.files;input.dispatchEvent(new this.EventCtor('input',{bubbles:true}));input.dispatchEvent(new this.EventCtor('change',{bubbles:true}));await wait(250);
 }
