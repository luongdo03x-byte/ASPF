...
export class FlowAdapter {
 ...
 settingsButton(){const buttons=this.composerButtons();return buttons.find(b=>/settings?|options?|preferences?|generation settings|tune|adjust|cài đặt|tùy chọn/i.test(semanticText(b)));}
 outputLabels(){const pattern=/number of outputs|outputs? per|outputs?|số lượng đầu ra|số ảnh|số kết quả/i;return [...(this.document.querySelectorAll?.('div,span,p,label,legend')||[])].filter(el=>pattern.test(semanticText(el)));}
 outputControlRoot(){for(const label of this.outputLabels()){let node=label.parentElement||null;for(let depth=0;node&&depth<5;depth++,node=node.parentElement){const controls=[...(node.querySelectorAll?.('button,[role="radio"],[role="option"],[role="combobox"],input[type="radio"],select')||[])];if(controls.length)return {label,node,controls};}}return null;}
 controlValue(el){const direct=el?.value;if(direct!=null&&String(direct).trim()!=='')return String(direct).trim();const attr=el?.getAttribute?.('value');if(attr!=null&&String(attr).trim()!=='')return String(attr).trim();const visible=String(el?.innerText||el?.textContent||'').trim();if(visible)return visible;return String(semanticText(el)||'').trim();}
 isSelected(el){return el?.checked===true||el?.selected===true||el?.getAttribute?.('aria-checked')==='true'||el?.getAttribute?.('aria-pressed')==='true'||el?.getAttribute?.('data-state')==='checked'||el?.getAttribute?.('data-selected')==='true';}
 exactOutputChoice(controls,count){const exact=String(count);return controls.find(el=>{const value=this.controlValue(el);if(value===exact)return true;const text=norm(semanticText(el));return new RegExp(`^${exact}\\s*(output|outputs|image|images|ảnh)?import { ExtensionError } from '../core/errors.js';
...
,'i').test(text);});}
 async waitForOutputControl(timeoutMs=2500){const start=Date.now();while(Date.now()-start<timeoutMs){const root=this.outputControlRoot();if(root)return root;await wait(100);}return null;}
 async ensureOutputCount(count=1){count=Number(count);if(!Number.isFinite(count)||count<1)throw new ExtensionError('INVALID_OUTPUT_COUNT',`Invalid output count: ${count}`,{retryable:false,stage:'prepare'});if(this.confirmedOutputCount===count)return;let root=this.outputControlRoot();if(!root){const settings=this.settingsButton();if(!settings)throw new ExtensionError('SETTINGS_BUTTON_NOT_FOUND','Flow composer Settings/Options button not found; refusing to generate with unknown output count',{retryable:false,stage:'prepare'});settings.click();root=await this.waitForOutputControl();}if(!root)throw new ExtensionError('OUTPUT_COUNT_CONTROL_NOT_FOUND','Flow Number of outputs setting not found; refusing to generate multiple outputs',{retryable:false,stage:'prepare'});const select=root.controls.find(el=>String(el.tagName||'').toLowerCase()==='select');if(select){if(String(select.value)!==String(count)){select.value=String(count);select.dispatchEvent?.(new this.EventCtor('input',{bubbles:true}));select.dispatchEvent?.(new this.EventCtor('change',{bubbles:true}));}if(String(select.value)!==String(count))throw new ExtensionError('OUTPUT_COUNT_NOT_CONFIRMED',`Flow output count did not change to ${count}`,{retryable:false,stage:'prepare'});this.confirmedOutputCount=count;return;}let choice=this.exactOutputChoice(root.controls,count);if(!choice)throw new ExtensionError('OUTPUT_COUNT_OPTION_NOT_FOUND',`Flow output option ${count} not found`,{retryable:false,stage:'prepare'});if(!this.isSelected(choice))choice.click();await wait(120);this.confirmedOutputCount=count;}
