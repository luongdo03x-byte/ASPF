...
export class FlowAdapter {
 ...
 addMediaButton(){const buttons=this.composerButtons();const labeled=buttons.find(b=>/add|attach|upload|plus|thêm|đính kèm/i.test(norm(semanticText(b))));if(labeled)return labeled;const geometric=this.nearEditorButtons();return geometric[0];}
 uploadMenuButton(){const nodes=[...(this.document.querySelectorAll?.('button,[role="menuitem"],[role="option"],[role="button"]')||[])];const patterns=['upload image','upload','add image','image','media','file','computer','device','ảnh','tệp','tải lên'];return nodes.find(el=>{if(el.disabled)return false;const t=norm(semanticText(el));return patterns.some(p=>t.includes(norm(p)))&&!/agent|settings|option|send|generate/.test(t);});}
 ...
 async uploadReference(file){
  ...
  if(!input){const add=this.addMediaButton();if(add){add.click();input=await this.waitForReferenceInput(1200);}}
  if(!input){const menu=this.uploadMenuButton();if(menu){menu.click();input=await this.waitForReferenceInput(1600);}}
  if(!input)throw new ExtensionError('REFERENCE_UPLOAD_UNAVAILABLE','Flow reference uploader did not appear after opening the Add menu',{retryable:true,stage:'reference'});
