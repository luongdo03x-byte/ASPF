...
export class FlowAdapter {
 ...
 settingsButton(){
  const buttons=this.composerButtons();
  return buttons.find(b=>{const t=norm(semanticText(b));return t.includes('settings')||t.includes('options')||t.includes('cài đặt')||t.includes('tùy chọn');});
 }
 outputLabel(){
 ...
 async prepare(job={}){await this.ensureReady();if(job.outputs===1)await this.ensureOneOutput();}
