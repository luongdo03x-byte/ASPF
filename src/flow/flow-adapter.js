...
export class FlowAdapter {
 ...
 referenceInput(){const inputs=[...(this.document.querySelectorAll?.('input[type="file"]')||[])].filter(x=>!x.disabled);return inputs.find(x=>/image/i.test(String(x.getAttribute?.('accept')||'')))||inputs[0];}
 addMediaButton(){const buttons=this.composerButtons();const labeled=buttons.find(b=>/(^|\s)(add|attach|upload|plus|thêm|đính kèm)(\s|$)/i.test(norm(semanticText(b))));if(labeled)return labeled;const all=this.nearEditorButtons();const agent=all.find(b=>norm(semanticText(b)).includes('agent'));const ar=this.buttonRect(agent);if(agent&&ar){const left=all.filter(b=>b!==agent).map(b=>({b,r:this.buttonRect(b)})).filter(x=>x.r&&x.r.cx<ar.cx&&Math.abs(x.r.cy-ar.cy)<=Math.max(24,ar.height*1.5)).map(x=>({b:x.b,gap:ar.cx-x.r.cx})).filter(x=>x.gap>0&&x.gap<=100).sort((a,b)=>a.gap-b.gap);if(left.length)return left[0].b;}return undefined;}
 uploadMenuButton(){const nodes=[...(this.document.querySelectorAll?.('[role="menuitem"],[role="option"],[role="menu"] button,[role="listbox"] button')||[])];const patterns=['upload image','upload','add image','image','media','file','computer','device','ảnh','tệp','tải lên'];return nodes.find(el=>{if(el.disabled)return false;const t=norm(semanticText(el));return patterns.some(p=>t.includes(norm(p)))&&!/agent|settings|option|send|generate|home|project/.test(t);});}
 async waitForReferenceInput(timeoutMs=2200){const start=Date.now();while(Date.now()-start<timeoutMs){const input=this.referenceInput();if(input)return input;await wait(100);}return null;}
 async uploadReference(file){
  if(!this.isProjectPage())throw new ExtensionError('FLOW_PROJECT_REQUIRED_REFERENCE','Flow project is no longer open before reference upload',{retryable:false,stage:'reference'});
  if(!(file instanceof File))throw new ExtensionError('REFERENCE_NOT_FILE','Reference payload is not a browser File',{retryable:false,stage:'reference'});
  if(!this.DataTransferCtor)throw new ExtensionError('DATATRANSFER_UNAVAILABLE','Browser DataTransfer is unavailable',{retryable:false,stage:'reference'});
  let input=this.referenceInput();
  if(!input){const add=this.addMediaButton();if(add){add.click();await wait(120);if(!this.isProjectPage())throw new ExtensionError('REFERENCE_NAVIGATED_AWAY','Reference Add control navigated away from the Flow project',{retryable:false,stage:'reference'});input=await this.waitForReferenceInput(1200);}}
  if(!input){const menu=this.uploadMenuButton();if(menu){menu.click();await wait(120);if(!this.isProjectPage())throw new ExtensionError('REFERENCE_NAVIGATED_AWAY','Reference upload menu navigated away from the Flow project',{retryable:false,stage:'reference'});input=await this.waitForReferenceInput(1600);}}
  if(!input)throw new ExtensionError('REFERENCE_UPLOAD_UNAVAILABLE','Flow reference uploader did not appear after opening the Add menu',{retryable:false,stage:'reference'});
  let dt;try{dt=new this.DataTransferCtor();dt.items.add(file);}catch(e){throw new ExtensionError('REFERENCE_DATATRANSFER_FAILED','Could not place reference image into DataTransfer: '+e.message,{retryable:false,stage:'reference'});}input.files=dt.files;input.dispatchEvent(new this.EventCtor('input',{bubbles:true}));input.dispatchEvent(new this.EventCtor('change',{bubbles:true}));await wait(250);
 }
