...
test('controller executes dependency chain, downloads each, completes then clears cache', async()=>{
 ...
 assert.equal(b.status,'COMPLETED'); assert.deepEqual(b.jobs.map(j=>j.status),['DONE','DONE']); assert.ok(calls.some(x=>x==='ref:S01_IMG01.png')); assert.equal(await cache.has('b1','S01_IMG01'),false);
});
